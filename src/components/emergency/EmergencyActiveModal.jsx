...
export function EmergencyActiveModal() {
  ...
  // If idle or cancelled, or if this is Secret SOS in stealth mode, do NOT show the obvious red modal
  if (sosState === 'idle' || sosState === 'cancelled') return null;
  if (isSecretSos) return null; // Secret SOS keeps the screen discreet

  // --- 1. Countdown Screen ---
  if (sosState === 'countdown' || sosState === 'activating') {
    return (
          ...
          {!isOnline && (
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs space-y-1.5">
              ...
              <p className="text-[11px] text-amber-800 leading-relaxed">
                Emergency broadcast saved locally. It will automatically transmit to the server as soon as connection is restored.
              </p>
            </div>
          )}
          ...
          {!showPinInput ? (
            <div className="pt-2">
              <Button
                ...
                fullWidth
                onClick={() => {
                  const pin = storage.getEmergencyPin() || '';
                  if (!pin) {
                    resolveSos('');
                  } else {
