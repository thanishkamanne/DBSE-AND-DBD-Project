...
export function SafetyProvider({ children }) {
  ...
  // Siren Lifecycle tied directly to SOS ACTIVE state:
  // SOS ACTIVE = siren continuously playing (unless secret SOS or disabled in settings)
  // SOS STOPPED / CANCELLED / RESOLVED / IDLE = siren immediately stopped
  useEffect(() => {
    if (sosState === 'active') {
      if (!isSecretSos && settings.sirenAllowed) {
  ...
  }, [sosState, isSecretSos, settings.sirenAllowed]);
  ...
  const startSosCountdown = useCallback((secret = false) => {
    ...
    // Crucial: Unlock browser audio context synchronously inside the user's initial interaction
  ...
  // Stop / Cancel SOS immediately
  const stopSos = useCallback(async (pin = '') => {
    return await resolveSos(pin);
  }, []);
